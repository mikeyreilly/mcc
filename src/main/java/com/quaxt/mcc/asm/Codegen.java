...
public class Codegen {
    ...
    private static Operand toOperand(ValIr val) {
        return switch (val) {
            ...
            case VarIr(String identifier) -> {
                ...
                yield t instanceof Array || t instanceof Structure ?
                        new PseudoMem(identifier, 0) : new Pseudo(identifier);
            }
    ...
    private static void codegenFunCall(FunCall funCall,
        ...
        if (funCall instanceof FunCall(String name, ArrayList<ValIr> args,
            ...
            int regIndex = 0;
            if (dst != null) {
                ReturnValueClassification r = classifyReturnValue((VarIr) dst);
                intDests = r.intDests;
    ...
    public static FunctionAsm convertFunction(FunctionIr functionIr) {
        ...
        for (Var param : functionType) {
            operands.add(new TypedOperand(toTypeAsm(param.type()),
                    new Pseudo(param.name())));
        }
...
//            ins.add(new Mov(returnType, retVal, returnType == DOUBLE ? XMM0
//            : AX));
        }
        ins.add(RET);
