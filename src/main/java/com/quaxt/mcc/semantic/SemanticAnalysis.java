    ...
    private static <T extends Statement> T loopLabelStatement(T statement,
        ...
        return switch (statement) {
            ...
            case For(ForInit init, Exp condition, Exp post, Statement body,
                ...
                Exp labeledCondition =
                        loopLabelStatement(condition, newLabel, newLabel);
                Exp labelledPost = loopLabelStatement(post, newLabel, newLabel);
                Statement labelledBody =
                        loopLabelStatement(body, newLabel, newLabel);
        ...
        for (MemberDeclaration member : structDecl.members()) {
            ...
            if (innerSd != null &&
                ...
                for (MemberEntry innerMember : innerSd.members()) {
                    sd =
                            addMemberToStruct(innerMember.name(),
                             innerMember.type(), sd,
toInteger(member.bitFieldWidth()));
                }
            } else {
                sd =
                        addMemberToStruct(member.name(), member.type(), sd,
                                toInteger(member.bitFieldWidth()));
            }
        }
        ...
        sd =
                new StructDef(sd.isUnion(), sd.alignment(), structSize,
 sd.members());
...
bitFieldWidth);
        ...
        if (last instanceof BitFieldMember lastBF &&
            ...
            if (lastBF.bitWidth() != 0 &&
                    lastEndBit + bitFieldWidth <= lastUnitBits) {
        ...
        switch (init) {
            ...
            case CompoundInit(ArrayList<Initializer> inits, Type type) -> {
                ...
                switch (targetType) {
                    ...
                    case Structure(boolean isUnion, String tag,
                        ...
                        if (initsSize > structDef.members().size() ||
                                (isUnion && initsSize > 1)) {
                            throw new Err("Too many elements in " +
                        ...
                        for (var initElement : inits) {
                            ...
                            if (member instanceof BitFieldMember bfm){
                                Constant z = (Constant) acc.getLast();
                                // keep just bitWidth bits
                                if (bfm.bitWidth() < 8 * Mcc.size(z.type())) {
                                    z = z.apply(BITWISE_AND,
                                            (1L << bfm.bitWidth()) - 1);
                                }
                                z = z.apply(SHL, bfm.bitOffset());
                                int accSize = acc.size();

                                acc.set(accSize - 1, (StaticInit) z);
                                if (i > 0 && byteOffset == previousByteOffset) {
                                    // smoosh last two members of acc together
                                    assert (accSize > 1);
                                    Constant<?> y =
                                            (Constant<?>) acc.get(accSize - 2);
                                    Constant<?> merged = z.apply(BITWISE_OR, y);
                                    acc.removeLast();
            ...
            case SingleInit(Exp exp, Type t) -> {
                if (exp instanceof Str(String s, Type _) &&
                        targetType instanceof Array(Type element,
                                                    Constant arraySize)) {
                    return handleStringLiteral(acc, s, element, arraySize,
false);
                } else if (exp instanceof Str(String s, Type _) &&
                    ...
                    StaticInit v = switch (exp) {
                        case Constant cc ->
                                (StaticInit) convertConst(cc, targetType);
...
//            if (inner instanceof Subscript(Var e1, Exp e2, Type _) &&
//                    evaluateExpAsConstant(e2) instanceof Constant c) {
//                return new PointerInit(e1.name(), c.toLong());
//            }
//            if (inner instanceof Subscript(Exp e2, Var e1, Type _) &&
//                    evaluateExpAsConstant(e2) instanceof Constant c) {
//                return new PointerInit(e1.name(), c.toLong());
//
...
//            } else {
//                var t = typeCheckExpression(exp);
//                AddrOf addr = (AddrOf) t;
//                return foo(addr.exp(), 0);
//            }
//        }
        if (evaluateExpAsConstant(exp) instanceof Constant c) {
            return (StaticInit) convertConst(c, targetType);
        }
        if (exp instanceof Var(String name, Type type) &&
                targetType instanceof Pointer(Type referenced) &&
                referenced instanceof FunType) {
            return new PointerInit(name);
        }
        return staticPointerInit(typeCheckExpression(exp), 0);
    }

    private static PointerInit staticPointerInit(Exp e, long offset) {
        if (e instanceof Subscript(Exp e1, Exp e2, Type type)) {
            Pointer p;
            Exp array=null;
            Exp subscript=null;
            if (e1.type() instanceof Pointer p1){
                p=p1;
                array=e1;
                subscript = e2;
            } else if (e2.type() instanceof Pointer p1){
                p=p1;
                array=e2;
                subscript = e1;
            } else throw new Err("Non constant initializer");
            long elementSize = size(type);


            if (evaluateExpAsConstant(subscript) instanceof Constant c)
                return staticPointerInit(array, offset + c.toLong() * elementSize);
            throw new Err("Non constant initializer");
        }
        if (e instanceof AddrOf(Exp exp, Type _)) {
            return staticPointerInit(exp, offset);
        }
        if (e instanceof Var(String name, Type _)) {
            return new PointerInit(name, offset);
        }
        throw new Todo();
    }
    ...
    /* returns the length of the String literal including trailing zero byte
    (s)*/
    private static Constant handleStringLiteral(List<StaticInit> acc,
                                                String s,
                                                Type element,
                                                Constant arrayLen,
                                                boolean pointerify) {
        if (!element.isCharacter()) throw new Err(
                "Can't initialize non-character type with a string " +
                        "literal");
        ...
        long zeroCount =
                arrayLen != null ? arrayLen.toLong() - s.length() - 1 : 1;
        if (pointerify) {
...
//            }
        ...
        if (varType instanceof Pointer(Type referenced) && referenced == CHAR &&
                initialValue instanceof Initial(List<StaticInit> staticInits) &&
    ...
    private static Function typeCheckFunctionDeclaration(Function decl,
        ...
        for (int i = 0; i < oldParams.size(); i++) {
            ...
            if (!isComplete(p) && !isComplete(decayArrayType(p)) &&
                    decl.body != null) {
        ...
        if (decl.body != null) {
            for (int i = 0; i < decl.parameters.size(); i++) {
                ...
                SYMBOL_TABLE.put(param.name(),
                 new SymbolTableEntry(adjustedParams.get(i), LOCAL_ATTR));
            }
...
enclosingFunction));
            case CaseStatement(Switch enclosingSwitch, Constant<?> label,
                ...
                if (label != null) {
                    ...
                    convertedConst =
                            (Constant<?>) convertConst(label, switchExpType);
            ...
            case DoWhile(Statement whileBody, Exp condition, String label) ->
                    new DoWhile((Statement) typeCheckBlockItem(whileBody,
                     enclosingFunction),
                      requireScalar(typeCheckAndConvert(condition)), label);
            case For(ForInit init, Exp condition, Exp post, Statement body,
...
typeCheckAndConvert(post),
                       (Statement) typeCheckBlockItem(body,
enclosingFunction), label);
            ...
            case Switch switchStatement -> {
                var typedExp =
                        (Exp) typeCheckBlockItem(switchStatement.exp,
                         enclosingFunction);
    ...
    private static VarDecl typeCheckLocalVariableDeclaration(VarDecl _decl) {
        ...
        if (init != null) {
            ...
            // typeCheckInit
            // to find the type of the var we're declaring in case it is used
            // in init
        ...
        if (!isComplete(varType)) {
            if (decl.storageClass() != EXTERN)
                fail("Attempt to declare variable of incomplete type with " +
                        "non-external storage class");
            if (init != null)
        ...
        if (decl.storageClass() == EXTERN) {
            ...
            convertCompoundInitializerToStaticInitList(init, varType,
             staticInits);
            initialValue = new Initial(staticInits);
...
new StaticAttributes(initialValue, false, decl.storageClass())));
    ...
    private static String addStringToSymbolTable(Type referenced,
                                                 List<StaticInit> staticInits) {
        String uniqueName = makeTemporary("string.");
        SYMBOL_TABLE.put(uniqueName,
                new SymbolTableEntry(new Array(referenced,
          new IntInit((int) strlen(staticInits))),
            ...
            case SingleInit(Exp exp, Type _) -> {
                if (exp instanceof Str(String s, Type _) &&
                        targetType instanceof Array(Type element,
                                                    Constant arraySize)) {
                    if (!element.isCharacter()) throw new Err(
                            "Can't initialize non-character type " +
    ...
    private static Exp convertByAssignment(Exp e, Type targetType) {
        ...
        if (e.type() instanceof FunType) {
            if (e instanceof Conditional(Exp condition, Exp ifTrue, Exp ifFalse,
                                         Type type)) {
                ifTrue =
                        isNullPointerConstant(ifTrue) ? ifTrue :
 typeCheckExpression(new AddrOf(ifTrue, null));
                ifFalse =
                        isNullPointerConstant(ifFalse) ? ifFalse :
                typeCheckExpression(new AddrOf(ifFalse, null));
        ...
        if ((isArithmeticType(t) && isArithmeticType(targetType)) ||
                (isNullPointerConstant(e) && targetType instanceof Pointer))
            return convertTo(e, targetType);
        if (targetType instanceof Pointer(Type referenced) &&
                referenced == VOID && e.type() instanceof Pointer) {
            return convertTo(e, targetType);
        }
        if (e.type() instanceof Pointer(Type referenced) &&
     ...
     * and trailing arguments that have type float are promoted to double.
     * These are called the default
    ...
    public static Exp typeCheckExpression(Exp exp) {
        return switch (exp) {
            ...
            case BinaryOp(BinaryOperator op, Exp e1, Exp e2, Type _) when
            ...
            case BinaryOp(BinaryOperator op, Exp e1, Exp e2, Type _) when op ==
                    ADD -> {
                Exp typedE1 = typeCheckAndConvert(e1);
                Exp typedE2 = typeCheckAndConvert(e2);
...
commonType);
 ...
 promoteIfNecessary(typedE2), typedE1.type());
                ...
                if ((op == REMAINDER && (t1 == DOUBLE || t2 == DOUBLE)) ||
                        ((op == REMAINDER || op == DIVIDE || op == IMUL) &&
                                (t1 instanceof Pointer ||
                                        t2 instanceof Pointer)))
                    fail("invalid operands to binary " + op + " (have ‘" + t1 +
                            "’ " + "and" + " ‘" + t2 + "’");
                if ((op == BITWISE_AND || op == BITWISE_XOR ||
                        op == BITWISE_OR) && (t1 == DOUBLE || t2 == DOUBLE ||
                        t1 instanceof Pointer || t2 instanceof Pointer)) {
                    fail("invalid operands to binary " + op + " (have ‘" + t1 +
                            "’ " + "and" + " ‘" + t2 + "’");
                }
                if ((t1 instanceof Pointer || t2 instanceof Pointer) &&
                    ...
                    if (!t1.equals(t2)) throw new Err(
                            "can't apply " + op + " to " + t1 + " " + "and " +
                                    t2);
                    yield new BinaryOp(op, typedE1, typedE2, INT);
                }
            ...
            case Cast(Type type, Exp inner) -> {
                ...
                if (innerType instanceof FunType &&
                        type instanceof Pointer(Type r) &&
                        r instanceof FunType) {
                ...
                if ((type instanceof Array) ||
                        (type instanceof Pointer && innerType == DOUBLE ||
                                innerType instanceof Pointer && type == DOUBLE))
            ...
            case Conditional(Exp condition, Exp ifTrue, Exp ifFalse,
                ...
                if (t1.equals(t2)) {
                    ...
                    commonType =
                            getCommonPointerType(typedIfTrue, typedIfFalse);
                else if (isArithmeticType(t1) && isArithmeticType(t1))
                    commonType = getCommonType(t1, t2);
                else if (t1 instanceof Structure(boolean isUnion1, String tag1,
                                                 StructDef _) &&
                        t2 instanceof Structure(boolean isUnion2, String tag2,
                                                StructDef _) &&
                        tag1.equals(tag2)) {
                    commonType = t1;
                } else if (t1 instanceof FunType &&
                        isNullPointerConstant(typedIfFalse)) {
                    yield new Conditional(typedCondition, typedIfTrue,
                     typedIfFalse, t1);
                    ...
                    yield new Conditional(typedCondition, typedIfTrue,
                     typedIfFalse, t2);
                        ...
                        "Can't convert branches of conditional to " + "a" +
                                " common type");
                yield new Conditional(typedCondition, convertTo(typedIfTrue,
                 commonType), convertTo(typedIfFalse, commonType), commonType);
            }
            ...
            case BuiltInFunctionCall(BuiltInFunction name, List<Exp> args,
                ...
                if (paramsSize != args.size()) fail("Function " + name +
            ...
            case UnaryOp(UnaryOperator op, Exp e1, Type _) when
                    op == POST_INCREMENT || op == POST_DECREMENT -> {
                Exp typedE1 = typeCheckAndConvert(e1);
                Type t1 = typedE1.type();
                if (isLvalue(typedE1) &&
                        (isArithmeticType(t1) || isPointerToComplete(t1))) {
            ...
            case Subscript(Exp e1, Exp e2, Type oldType) -> {
                ...
                if (t1 instanceof Pointer p && t2.isInteger() &&
                        isPointerToComplete(t1)) {
                    ptrType = p;
                    typedE2 = convertTo(typedE2, LONG);
                } else if (t1.isInteger() && t2 instanceof Pointer p &&
                        isPointerToComplete(t2)) {
                    ptrType = p;
                    typedE1 = convertTo(typedE1, LONG);
                } else throw new Err(
                        "Subscript must have integer and pointer " +
                                "(to complete type) operands");
            ...
            case Arrow(Exp pointer, String member, Type _) -> {
                ...
                if (typedPointer.type() instanceof Pointer(Type structure) &&
                        structure instanceof Structure(boolean isUnion,
                                                       String tag,
                                                       StructDef _)) {
                    StructDef structDef = TYPE_TABLE.get(tag);
            ...
            case Generic(Exp controllingExp, ArrayList<Cast> genericAssocList,
                ...
                Type controllingType =
                        typeCheckExpression(controllingExp).type();
            ...
            case ExpressionStatement(Block block) ->
                    new ExpressionStatement(typeCheckBlock(block, null));
        };
    }
    ...
    private static boolean isLvalue(Exp exp) {
        if (exp instanceof BinaryOp binop &&
                binop.op() instanceof CompoundAssignmentOperator) return true;
        ...
        return exp instanceof Dereference || exp instanceof Var ||
                exp instanceof Subscript || exp instanceof Str ||
                exp instanceof Arrow;
    }
    ...
    public static Type getCommonType(Type t1, Type t2) {
        ...
        if (size(t1) == size(t2)) return t1.isSigned() ? t2 : t1;
    ...
    public static Type resolveType(Type typeSpecifier,
        ...
        return switch (typeSpecifier) {
            case Structure(boolean isUnion, String tag, StructDef sd) -> {
                ...
                if (e != null) {
                    if ((e.isUnion() == TagEntryType.UNION) != isUnion &&
                            tag != null) {
                        throw new Err("incompatible with earlier declaration " +
                                "of " + (e.isUnion() ==
                                TagEntryType.UNION ? "union" : "struct") + " " +
                                tag);
                    }
            ...
            case Pointer(Type referenced) -> {
                if (referenced instanceof Structure s) {
                    ifStructureResolveAsDeclaration(s, identifierMap,
                     structureMap, enclosingFunction);
                }
                referenced =
                        resolveType(referenced, identifierMap, structureMap,
                         enclosingFunction);
            ...
            case Array(Type element, Constant size) -> {
                if (size instanceof ConstantExp c) {
                    size =
                            new ConstantExp(resolveExp(c.exp(), identifierMap
                            , structureMap, enclosingFunction));
                }
            ...
            case Typeof(Exp exp) ->
                    new Typeof(resolveExp(exp, identifierMap, structureMap,
                            enclosingFunction));
...
resolveFileScopeVariableDeclaration(varDecl,
    ...
    public static EnumSpecifier resolveEnumSpecifier(EnumSpecifier decl,
        ...
        if (prevEntry != null && prevEntry.fromCurrentScope()) {
            if (prevEntry.isUnion() != TagEntryType.ENUM) {
                throw new Err("Attempt to redeclare " + prevEntry.isUnion() +
                        " as struct");
            }
        }
        if (prevEntry == null || !prevEntry.fromCurrentScope()) {
            uniqueTag = makeTemporary(decl.tag() + ".");
            structureMap.put(decl.tag(), new TagEntry(TagEntryType.ENUM,
             uniqueTag, true));
    ...
    private static StructOrUnionSpecifier resolveStructureDeclaration(
        ...
        if (prevEntry == null || !prevEntry.fromCurrentScope()) {
            uniqueTag = makeTemporary(decl.tag() + ".");
            structureMap.put(decl.tag(), new TagEntry(decl.isUnion() ?
             TagEntryType.UNION : TagEntryType.STRUCT, uniqueTag, true));
        } else {
            uniqueTag = prevEntry.name();
        }
    ...
    * */
    ...
    private static Function resolveFunctionDeclaration(Function function,
        ...
        List<Var> newArgs =
                resolveParams(function.parameters, innerMap,
        ...
        return switch (blockItem) {
            ...
            case Function function ->
                    resolveFunctionDeclaration(function, identifierMap,
                     structureMap);
            case StructOrUnionSpecifier structDecl ->
                    resolveStructureDeclaration(structDecl, identifierMap,
                            structureMap, enclosingFunction);
        };
    }
        ...
        return switch (blockItem) {
            ...
            case Exp exp ->
                    resolveExp(exp, identifierMap, structureMap,
                     enclosingFunction);
            case LabelledStatement(String label, Statement statement) ->
                    new LabelledStatement(labelForFunction(label,
                     enclosingFunction.name), resolveStatement(statement,
                      identifierMap, structureMap, enclosingFunction));
            case Break b -> {
            ...
            case CaseStatement(Switch enclosingSwitch, Constant<?> label,
                               Statement statement) ->
                    new CaseStatement(enclosingSwitch,
                            (Constant<?>) resolveExp(label, identifierMap,
                      structureMap, enclosingFunction),
...
enclosingFunction));
            case If(Exp condition, Statement ifTrue, Statement ifFalse) ->
                    new If(resolveExp(condition, identifierMap, structureMap,
                     enclosingFunction), resolveStatement(ifTrue,
                      identifierMap, structureMap, enclosingFunction),
                       resolveStatement(ifFalse, identifierMap, structureMap,
                        enclosingFunction));
            case Block block ->
                    resolveBlock(block, new ArrayList<>(),
                ...
                yield new For(resolveForInit(init, newVariableMap,
                 newStructureMap, enclosingFunction), resolveExp(condition,
                  newVariableMap, newStructureMap, enclosingFunction),
                   resolveExp(post, newVariableMap, newStructureMap,
                        enclosingFunction), resolveStatement(body, newVariableMap
            ...
            case Switch switchStatement -> {
                switchStatement.exp =
                        resolveExp(switchStatement.exp, identifierMap,
                                structureMap, enclosingFunction);
                switchStatement.body =
                        resolveStatement(switchStatement.body, identifierMap,
structureMap, enclosingFunction);
                yield switchStatement;
            }
            case BuiltinC23VaStart(Var ap) ->
 ...
 structureMap, enclosingFunction));
    ...
    private static Map<String, TagEntry> copyStructureMap(Map<String,
     TagEntry> m) {
        Map<String, TagEntry> copy = new DebugHashMap<>(m.size());
        for (var e : m.entrySet()) {
            var v = e.getValue();
            copy.put(e.getKey(), new TagEntry(v.isUnion(), v.name(), false));
        }
        return copy;
        ...
        Exp r = switch (exp) {
            ...
            case Assignment(Exp left, Exp right, Type type) ->
                    isLvalue(left) ? new Assignment(resolveExp(left,
                            identifierMap, structureMap, enclosingFunction),
                      resolveExp(right, identifierMap, structureMap,
 enclosingFunction), type) : fail("Invalid lvalue");
            case CompoundAssignment(CompoundAssignmentOperator op, Exp left,
                                    Exp right, Type tempType, Type type) ->
                    op instanceof CompoundAssignmentOperator &&
                            !isLvalue(left) ? fail("Invalid lvalue") :
                             new CompoundAssignment(op, resolveExp(left,
                              identifierMap, structureMap, enclosingFunction)
 , resolveExp(right, identifierMap, structureMap
 , enclosingFunction), tempType, type);
...
enclosingFunction), type);
            case ConstantExp(Exp cexp) ->
                    new ConstantExp(resolveExp(cexp, identifierMap,
                     structureMap, enclosingFunction));
            case Constant constant -> constant;
            case Str str -> str;
            case UnaryOp(UnaryOperator op, Exp arg, Type type) ->
                    (op == POST_INCREMENT || op == POST_DECREMENT) &&
                            !isLvalue(arg) ? fail("Invalid lvalue") :
 ...
 , structureMap, enclosingFunction);
            ...
            case Conditional(Exp condition, Exp ifTrue, Exp ifFalse,
                    ...
                    new Conditional(resolveExp(condition, identifierMap,
                     structureMap, enclosingFunction), resolveExp(ifTrue,
                      identifierMap, structureMap, enclosingFunction),
            ...
            case BuiltInFunctionCall(BuiltInFunction name, List<Exp> args,
                    ...
                    new BuiltInFunctionCall(name, resolveArgs(identifierMap,
                     structureMap, args, enclosingFunction), type);
            case Cast(Type type, Exp e) -> {
                Type resolvedType =
                        resolveType(type, identifierMap, structureMap,
                    enclosingFunction);
            ...
            case Subscript(Exp array, Exp index, Type type) ->
                    new Subscript(resolveExp(array, identifierMap,
                     structureMap, enclosingFunction), resolveExp(index,
                            identifierMap, structureMap, enclosingFunction), type);
            case SizeOf(Exp e) ->
            ...
            case Dot(Exp structure, String member, Type type) ->
                    new Dot(resolveExp(structure, identifierMap, structureMap
                    , enclosingFunction), member, type);
...
structureMap, enclosingFunction), resolvedType);
            ...
            case Generic(Exp controllingExp, ArrayList<Cast> genericAssocList,
                ...
                for (int i = 0; i < genericAssocList.size(); i++) {
                    ...
                    Type resolvedType =
                            resolveType(genericAssoc.type(), identifierMap,
structureMap, enclosingFunction);
                ...
                yield new Generic(resolveExp(controllingExp, identifierMap,
                 structureMap, enclosingFunction), genericAssocList,
                  resolveExp(defaultExp, identifierMap, structureMap,
                            enclosingFunction));
            }
            case ExpressionStatement(Block stmt) ->
                    new ExpressionStatement(resolveBlock(stmt,
                     new ArrayList<>(), identifierMap, structureMap,
                      enclosingFunction));
        };
        return r;
    }
